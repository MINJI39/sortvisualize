...
작성자 : 김민지
SelectionSort를 선택했을 때 visualize 시켜 보여주는 클래스 
		...
		if (usedbuffer) {
			...
			for (int i = 0; i < list.length; i++) {
				if (list[i] == 100) {
					...
					g.setColor(Color.WHITE);
					g.setFont(fontbig);
					g.drawString(Integer.toString(list[i]), 2 * BORDER_WIDTH + (columnWidth + 1) * i + 2,
							getHeight() - 2 * BORDER_WIDTH - 100);
				} else {
					g.setColor(Color.WHITE);
					g.setFont(fontbasic);
			...
			if (grayColumn < size) {
				for (int i = 0; i <= grayColumn; i++) {
					...
					g.setColor(Color.BLACK);
					g.setFont(fontbasic);
					g.drawString(Integer.toString(list[i]), 2 * BORDER_WIDTH + (columnWidth + 1) * i,
							getHeight() - 2 * BORDER_WIDTH - 100);
				}
			}
		} else {
