...
작성자 : 김민지
InsertionSort를 선택했을 때 visualize 시켜 보여주는 클래스 
	...
	private void paintingjob(Graphics g, int w, int h) {
		...
		if (usedbuffer) {
			...
			if (cyanColumn != -1) {
				...
				g.fillRect(2 * BORDER_WIDTH + (columnWidth + 1) * cyanColumn,
						getHeight() - list[cyanColumn] * columnHeight - 2 * BORDER_WIDTH - 100, columnWidth,
						(list[cyanColumn] + 1) * columnHeight);

				g.setColor(Color.WHITE);
				g.setFont(fontbasic);
				g.drawString(Integer.toString(list[cyanColumn]), 2 * BORDER_WIDTH + (columnWidth + 1) * cyanColumn + 1,
						getHeight() - 2 * BORDER_WIDTH - 100);
			}
		}
	}
